	...
	while more_recommendations:
		...
		if recommendation == None:
			more_recommendations = False
                        ...
                        solution = input_space.vectorify(recommendation[ 'params' ])
			...
			vsom_acum = 0.0
			for j in range(M):
	                        vs_acum += fun(solution, gradient = False)['shd']
	                        vsom_acum += fun(solution_om, gradient = False)['shd']
			values_solution = vs_acum/float(M)
			values_solution_om = vsom_acum/float(M)

			with open('value_solution.txt', 'a') as f:
                                print >> f, "%lf" % (values_solution)

                        with open('value_solution_om.txt', 'a') as f:
                                print >> f, "%lf" % (values_solution_om)

			with open('params.txt','a') as f_handle:
                                np.savetxt(f_handle, np.array([solution]), delimiter = ' ', newline = '\n')
